...
\hypersetup{unicode=true,
            pdftitle={Tables for Open Computer Science paper},
...
\setlength{\droptitle}{-2em}
  ...
  \title{Tables for Open Computer Science paper}
...
1:}\label{code-to-generate-latex-code-for-table-1}
...
\NormalTok{df2x <-}\StringTok{ }\NormalTok{df2 }\OperatorTok{%>%}\StringTok{ }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{text_spec}\NormalTok{(x, }\StringTok{"latex"}\NormalTok{, }\DataTypeTok{angle =} \DecValTok{90}\NormalTok{)) }


\CommentTok{# build table  }
\NormalTok{table2 <-}\StringTok{ }\NormalTok{df2x }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable}\NormalTok{(}\DataTypeTok{align=}\KeywordTok{c}\NormalTok{(}\StringTok{"c"}\NormalTok{, }\KeywordTok{rep}\NormalTok{(}\StringTok{"l"}\NormalTok{,}\DecValTok{5}\NormalTok{)), }\DataTypeTok{escape =}\NormalTok{ F, }\DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{caption =} \StringTok{"Overview of the regression coefficients of the Bayesian Generalized Linear Mixed-Models"}\NormalTok{, }\DataTypeTok{col.names =} \KeywordTok{c}\NormalTok{(}\StringTok{" "}\NormalTok{, }\StringTok{"Model with aggregation method for FaceReader's estimates"}\NormalTok{, }\StringTok{"Regression coefficient"}\NormalTok{, }\StringTok{"Credible interval"}\NormalTok{, }\StringTok{"Number of observations"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{column_spec}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DataTypeTok{width=}\StringTok{"0.8cm"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{column_spec}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DataTypeTok{width=}\StringTok{"7.0cm"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{column_spec}\NormalTok{(}\DecValTok{3}\NormalTok{, }\DataTypeTok{width=}\StringTok{"3.0cm"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{column_spec}\NormalTok{(}\DecValTok{4}\NormalTok{, }\DataTypeTok{width=}\StringTok{"2.5cm"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{column_spec}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DataTypeTok{width=}\StringTok{"1.8cm"}\NormalTok{) }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{footnote}\NormalTok{(}\DataTypeTok{general=}\StringTok{"Coefficients are based on standardized predictors (FaceReader), but unstandardized outcomes (self-reports). Some missing self-reports reduced the sample size of specific analyses."}\NormalTok{,}
           \DataTypeTok{title_format =} \KeywordTok{c}\NormalTok{(}\StringTok{"italic"}\NormalTok{), }\DataTypeTok{threeparttable =}\NormalTok{ T) }
\NormalTok{table2}
\end{Highlighting}
\end{Shaded}
